...
<b style="color:#e7e2d4">THE MARKET.</b> You start with 1,000,000 CRED and earn more just by leaving this running. Tap [ SELL ] to list a card to the house collectors; bids come in over time; accept one and the card is destroyed for CRED. A card is worth more the higher it ranked, the wilder its stats, and the longer you have held it, so holding pays.<br><br>
<b style="color:#e7e2d4">THE OPEN MARKET.</b> Tap [ POST ] to list a card for the whole room, not just the house. It opens a GitHub issue, which is the shared ledger (no server, the same trick the arena uses). Real collectors bid in the thread; once you agree on a price, you send them the trade code and they REDEEM it. The room sets the real value.
</div></details>
...
<div style="font-size:10px;letter-spacing:.16em;color:#8d8a7c;margin:16px 0 6px">THE OPEN MARKET // REAL COLLECTORS, POSTED ON GITHUB, NO SERVER</div>
<div id="floorpublic"><span class="sub">loading live listings...</span></div>
...
(function(){
 ...
 Promise.all([
  ...
  loadPublic();setInterval(loadPublic,90000);
 ...
 function tradeAway(code){
  ...
  if(!window.confirm('Trade away '+card.name+'? It leaves your collection and you get a one-time code to give someone.'))return;
  ...
  el.innerHTML='Traded away <b>'+esc(card.name)+'</b>. Give this code to anyone; they paste it into REDEEM to receive the card. It is gone from your collection.<br><textarea readonly id="tcval" style="width:100%;height:48px;margin-top:6px;background:#0d0f0d;color:#cfe7b6;border:1px solid #34362f;font:inherit;font-size:10px;padding:6px;resize:vertical">'+esc(blob)+'</textarea><button id="tccopy" type="button" class="listen" style="margin-top:5px">[ COPY CODE ]</button>';
  var cp=document.getElementById('tccopy');if(cp)cp.onclick=function(){var ta=document.getElementById('tcval');ta.select();var ok=false;try{ok=document.execCommand('copy');}catch(e){}if(navigator.clipboard)navigator.clipboard.writeText(blob);this.textContent='[ COPIED ]';};
 }
 function redeem(str){
  var el=document.getElementById('tradecode');
  str=(str||'').trim();if(!str)return;
  var card;try{card=JSON.parse(atob(str));}catch(e){card=null;}
  if(!card||!card.code||!card.name){if(el){el.style.display='block';el.innerHTML='That is not a valid trade code.';}return;}
  if(cardByCode(card.code)){if(el){el.style.display='block';el.innerHTML='You already hold <b>'+esc(card.name)+'</b>.';}return;}
  cards.unshift(card);cards=cards.slice(0,60);save();renderCollection();
  if(el){el.style.display='block';el.innerHTML='Redeemed <b>'+esc(card.name)+'</b> into your collection.';}
 ...
 // THE OPEN MARKET. Real cross-user trading with no server: GitHub Issues are the
 // shared ledger (the same trick the Arena uses). You POST a listing (opens an
 // issue), everyone reads it, collectors bid in the thread, and the card itself
 // transfers with the existing trade code once you agree. Fully serverless.
 function mkHandle(){var h='';try{h=localStorage.getItem('signal_handle')||'';}catch(e){}if(!h){h=((window.prompt('Pick a collector handle (shown on your listings and bids):','')||'').trim()).slice(0,24);if(h){try{localStorage.setItem('signal_handle',h);}catch(e){}}}return h||'ANON';}
 function ghNew(label,title,body){if(!MKT_REPO){alert('Open market needs the repo configured.');return;}window.open('https://github.com/'+MKT_REPO+'/issues/new?labels='+encodeURIComponent(label)+'&title='+encodeURIComponent(title)+'&body='+encodeURIComponent(body),'_blank','noopener');}
 function postPublic(code){var c=cardByCode(code);if(!c)return;var h=mkHandle();var rk=c.resolved?(c.resolved.legend?'LEGEND':(c.resolved.podium?'RARE':'CARD')):'ROOKIE';var fin=(FIN[c.finish]&&FIN[c.finish].n)||'';var board=location.origin+location.pathname;ghNew('card-trade','card for trade: '+c.name+(fin?(' ('+fin+')'):''),'Listing a card on THE SIGNAL open market.\n\nCARD-LIST '+c.code+'|'+c.name+'|'+rk+'|'+fin+'|'+cardValue(c)+'|'+h+'\n\nThe board: '+board+'\nBidders, comment with a line: CARD-BID '+c.code+'|<amount>|<your handle>\nWhen we agree I send the trade code and you REDEEM it on the board. The card is one-of-one.');}
 function bidPublic(serial,name){var amt=((window.prompt('Your bid for '+name+' in CRED:','')||'').replace(/[^0-9]/g,''));if(!amt)return;var h=mkHandle();ghNew('card-trade','bid on '+name,'Bidding on a card on THE SIGNAL open market.\n\nCARD-BID '+serial+'|'+amt+'|'+h+'\n\nSeller: reply here and send the trade code when we agree.');}
 function loadPublic(){
  var el=document.getElementById('floorpublic');if(!el)return;
  if(!MKT_REPO){el.innerHTML='<span class="sub">Open market unavailable (no repo configured).</span>';return;}
  fetch('https://api.github.com/repos/'+MKT_REPO+'/issues?labels=card-trade&state=all&per_page=100',{headers:{Accept:'application/vnd.github+json'}}).then(function(r){return r.ok?r.json():[];}).then(function(issues){
   var lists={},bids={};
   (issues||[]).forEach(function(it){var b=it.body||'';
    var lm=b.match(/CARD-LIST\s+([^\n|]+)\|([^\n|]+)\|([^\n|]+)\|([^\n|]*)\|([^\n|]+)\|([^\n]+)/);
    if(lm){lists[lm[1].trim()]={serial:lm[1].trim(),name:lm[2].trim(),rank:lm[3].trim(),finish:lm[4].trim(),ask:lm[5].trim(),seller:lm[6].trim(),url:it.html_url};}
    var bm=b.match(/CARD-BID\s+([^\n|]+)\|([^\n|]+)\|([^\n]+)/);
    if(bm){var sr=bm[1].trim();(bids[sr]=bids[sr]||[]).push({amount:parseInt(bm[2].replace(/[^0-9]/g,''),10)||0,bidder:bm[3].trim()});}
   });
   var keys=Object.keys(lists);
   if(!keys.length){el.innerHTML='<span class="sub">No public listings yet. Tap POST on one of your cards to list it for the whole room. It opens a GitHub issue, which is the shared ledger (no server). Collectors bid in the thread; once you agree, send the trade code and they REDEEM it above.</span>';return;}
   el.innerHTML=keys.map(function(k){var L=lists[k];var bs=(bids[k]||[]).sort(function(a,b){return b.amount-a.amount;});var top=bs.length?('top bid <b style="color:#ffd56b">'+fmt(bs[0].amount)+'</b> CRED by '+esc(bs[0].bidder)):'no bids yet';
    return '<div style="border:1px solid #34362f;padding:10px;margin:6px 0"><div style="display:flex;justify-content:space-between;gap:8px;align-items:center"><b>'+esc(L.name)+'</b><span style="color:#6f6c5f;font-size:9px">'+esc(L.rank)+(L.finish?(' // '+esc(L.finish)):'')+'</span></div><div style="color:#a9a596;margin:4px 0">ask '+esc(L.ask)+' CRED // '+top+' // seller '+esc(L.seller)+'</div><div style="display:flex;gap:6px;flex-wrap:wrap"><button class=tbtn data-bid="'+esc(L.serial)+'~'+esc(L.name)+'" type="button">[ BID ]</button><a class=tbtn href="'+esc(L.url)+'" target="_blank" rel="noopener" style="text-decoration:none">[ THREAD ]</a></div></div>';
   }).join('');
   var bb=el.querySelectorAll('[data-bid]');for(var i=0;i<bb.length;i++)bb[i].addEventListener('click',function(){var p=this.getAttribute('data-bid').split('~');bidPublic(p[0],p[1]);});
  }).catch(function(){el.innerHTML='<span class="sub">Could not reach the open market right now.</span>';});
 }
 ...
 function drawCards(){
  ...
  el.innerHTML=cards.map(function(c){
   ...
   return '<div class="'+cls+'"><canvas class=emb data-code="'+esc(c.code)+'" width=156 height=74></canvas><span class=rk>'+rk+'</span><span class=nm>'+esc(c.name)+'</span>'+finBadge(c.finish)+'<div>'+esc(c.house||'')+' // '+(c.fade?'FADE':'TAIL')+'</div>'+hist+'<div class=code>'+esc(c.code||'')+' // est '+fmt(cardValue(c))+' CRED</div><div style="margin-top:5px;display:flex;gap:5px;flex-wrap:wrap"><button class=tbtn data-trade="'+esc(c.code)+'" type="button">[ TRADE ]</button>'+act+'<button class=tbtn data-public="'+esc(c.code)+'" type="button">[ POST ]</button></div></div>';
  }).join('');
  ...
  var ps=el.querySelectorAll('[data-public]');for(var p=0;p<ps.length;p++)ps[p].addEventListener('click',function(){postPublic(this.getAttribute('data-public'));});
 }
 function drawRafters(){
  var el=document.getElementById('floorrafters');if(!el)return;
  if(!rafters.length){el.innerHTML='<span class="sub">Empty. The first champion is crowned at the bell.</span>';return;}
  el.innerHTML=rafters.map(function(r,idx){var lg=r.rarity==='LEGEND',cls='lcard'+(lg?' legend':' rare');return '<div class="'+cls+'"><canvas class=remb data-i="'+idx+'" width=156 height=74></canvas><span class=rk>'+r.rarity+' // SEASON '+r.season+' #'+r.rank+'</span><span class=nm>'+esc(r.name)+'</span><div>'+esc(r.house||'')+' // '+(r.fade?'FADE':'TAIL')+'</div><div style="margin-top:4px">net '+fmt(r.finalBank)+' // W'+(r.maxStreak||0)+' // big +'+fmt(r.biggestWin)+'</div></div>';}).join('');
  var cvs=el.querySelectorAll('canvas.remb');for(var i=0;i<cvs.length;i++){var r=rafters[parseInt(cvs[i].getAttribute('data-i'),10)];if(r)drawEmblem(cvs[i],{code:r.name+r.season+r.rank,risk:null},r.rarity==='LEGEND'?'legend':'rare');}
 }
