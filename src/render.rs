...
(function(){
 ...
 function drawRafters(){
  ...
  el.innerHTML=rafters.map(function(r,idx){var lg=r.rarity==='LEGEND',cls='lcard'+(lg?' legend':' rare');return '<div class="'+cls+'"><canvas class=remb data-i="'+idx+'" width=164 height=86></canvas><span class=rk>'+r.rarity+' // SEASON '+r.season+' #'+r.rank+'</span><span class=nm>'+esc(r.name)+'</span><div>'+esc(r.house||'')+' // '+(r.fade?'FADE':'TAIL')+'</div><div style="margin-top:4px">net '+(r.finalBank>=1e13?'&infin;':fmtAbbr(r.finalBank))+' // W'+(r.maxStreak||0)+' // big +'+fmtAbbr(r.biggestWin)+'</div></div>';}).join('');
