...
/// An empty observatory: the manifold is undefined for every term, so ranking
/// behaves exactly as the momentum-times-weight baseline (no boost).
fn obs() -> Observatory {
    Observatory {
        today: "2026-06-17".into(),
...
fn caps_and_dedups() {
    ...
    let picks = rank_and_select(sigs, 7, 4, &HashMap::new(), &obs());
    assert!(picks.len() <= 4);
...
fn learned_weights_change_the_order() {
    ...
    let picks = rank_and_select(sigs, 1, 1, &w, &obs());
...
fn empty_in_empty_out() {
    assert!(rank_and_select(vec![], 1, 4, &HashMap::new(), &obs()).is_empty());
}
